...
def save_year_month_figures(df: pd.DataFrame) -> None:
    ...
    month["period"] = (
        month["year"].astype(str) + "-" + month["month"].astype(str).str.zfill(2)
    )
    px.line(
        month, x="period", y="rows", title="Tagged corpus coverage by month"
    ).write_html(
...
def dataset_bias(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows.append(
        {"issue": "year_min", "value": int(df["year"].min()), "severity": "info"}
    )
    rows.append(
        {"issue": "year_max", "value": int(df["year"].max()), "severity": "info"}
    )
    rows.append(
        {
            "issue": "largest_year_share",
            "value": float(df["year"].value_counts(normalize=True).max()),
            "severity": "P1",
        }
    )
    macro_counts = {}
    ...
    if macro_counts:
        rows.append(
            {
                "issue": "largest_macro_share",
                "value": max(macro_counts.values()) / len(df),
                "severity": "P1",
            }
        )
        ...
        records.append(
            {
                ...
                "similar_id": None,
                "score": 100,
                "text_preview": truncate_text(row["text"], 220),
            }
        )
...
def length_tables(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in df.iterrows():
        for macro in parse_json_list(row["macro_tags"]):
            rows.append(
                {
                    "macro_tag": macro,
                    "text_length_chars": row["text_length_chars"],
                    "text_length_words": row["text_length_words"],
                }
            )
    macro_df = pd.DataFrame(rows)
    by_macro = (
        macro_df.groupby("macro_tag")
        .agg(
    ...
    px.histogram(
        df, x="text_length_chars", nbins=60, title="Text length distribution"
    ).write_html(
...
def structural_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["line_count"] = (
        result["text"].fillna("").astype(str).map(lambda x: max(1, x.count("\n") + 1))
    )
    result["sentence_count"] = (
        result["text"]
        .fillna("")
        .astype(str)
        .map(lambda x: max(1, len(re.findall(r"[.!?]+", x))))
    )
    result["dash_count"] = (
        result["text"].fillna("").astype(str).map(lambda x: x.count("-") + x.count("—"))
    )
    result["question_count"] = (
        result["text"].fillna("").astype(str).map(lambda x: x.count("?"))
    )
...
def interpretability_summary(
    ...
    for cluster, part in df.groupby("cluster_leiden"):
        ...
        dominant = (
            pur["dominant_macro_tag"].iloc[0]
            if not pur.empty and "dominant_macro_tag" in pur
            else None
        )
        share = (
            float(pur["dominant_macro_share"].iloc[0])
            if not pur.empty and "dominant_macro_share" in pur
            else 0.0
        )
        ...
        rows.append(
            {
                ...
                "central_example_ids": ", ".join(
                    central.loc[central["cluster_leiden"] == cluster, "id"]
                    .astype(str)
                    .head(5)
                ),
                "borderline_example_ids": ", ".join(
                    borderline.loc[borderline["cluster_leiden"] == cluster, "id"]
                    .astype(str)
                    .head(5)
                ),
                "caveat": (
                    "Mixed tags and humor formulas may blur this cluster."
                    if confidence != "high"
                    else "Dominant macro tag is relatively concentrated."
                ),
            }
        )
...
def robustness_summary(tables_dir: str | Path) -> pd.DataFrame:
    ...
    if grid_path.exists():
        ...
        rows.append(
            {
                "issue": "cluster_count_min",
                "value": int(grid["cluster_count"].min()),
                "severity": "info",
            }
        )
        rows.append(
            {
                "issue": "cluster_count_max",
                "value": int(grid["cluster_count"].max()),
                "severity": "P1",
            }
        )
        rows.append(
            {
                "issue": "largest_cluster_share_max",
                "value": float(grid["largest_cluster_share"].max()),
                "severity": "P1",
            }
        )
    if pair_path.exists():
        ...
        rows.append(
            {
                "issue": "stability_ari_mean",
                "value": float(pair["ari"].mean()),
                "severity": "P1",
            }
        )
        rows.append(
            {
                "issue": "stability_ami_mean",
                "value": float(pair["ami"].mean()),
                "severity": "P1",
            }
        )
    return pd.DataFrame(rows)
