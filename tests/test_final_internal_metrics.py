...
def test_final_internal_metrics_reflect_final_clustering():
    ...
    assert math.isclose(float(row["largest_cluster_share"]), 0.12906153566890544, rel_tol=0, abs_tol=1e-9)
...
def test_legacy_internal_metrics_now_points_to_final_and_initial_is_archived():
    ...
    assert int(current_metrics.iloc[0]["cluster_count"]) == int(final_metrics.iloc[0]["cluster_count"]) == 20
