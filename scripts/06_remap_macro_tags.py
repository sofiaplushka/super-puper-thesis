...
def count_macros(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(
            [
                {"macro_tag": k, "count": v, "share": v / total_rows}
...
def write_mapping_csv(mapping_path: str) -> None:
    ...
    for macro, tags in mapping.items():
        for tag in tags or []:
            records.append(
                {"macro_tag": macro, "tag_raw": tag, "tag_norm": normalize_tag(tag)}
            )
...
def write_audit(before: pd.DataFrame) -> None:
    ...
    nb = json.loads(
        Path("notebooks/tagged_corpus_analysis.ipynb").read_text(encoding="utf-8")
    )
    executed = sum(
...
def main() -> int:
    ...
    if "share" not in before.columns:
        before["share"] = (
            before["count"] / pd.read_csv("data/processed/anekdots_tagged.csv").shape[0]
        )
    write_audit(before)
    ...
    for path in [
        ...
        if Path(path).exists():
            ...
            remapped.to_csv(path, index=False, encoding="utf-8", lineterminator="\n")
    ...
    if unmapped_after.empty:
        ...
        unmapped_summary = (
            unmapped_after.groupby("tag_raw", as_index=False)
            .size()
    ...
    before_cmp = before.rename(
        columns={"count": "count_before", "share": "share_before"}
    )
    ...
    lines = [
        ...
        (
            unmapped_summary.head(40).to_markdown(index=False)
            if not unmapped_summary.empty
            else "No unmapped tags remain in the current corpus."
        ),
    ]
    ...
    append_progress(
        f"Phase 2 remap complete: other {int(other_before)} -> {int(other_after)} ({reduction:.1%} reduction)."
    )
    print(
        {
            "other_before": int(other_before),
            "other_after": int(other_after),
            "reduction": reduction,
            "unmapped_after": len(unmapped_summary),
        }
    )
    return 0
